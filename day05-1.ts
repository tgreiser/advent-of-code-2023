...
export class Day05 {
    ...
    process(): number {
        ...
        for (let iX = 0; iX < this.seeds.length; iX++) {
            let found = this.lookupSeed(this.seeds[iX]);
            console.log(`seed: ${this.seeds[iX]}, found: ${found}\n\n`);
            results.push(found);
            if (found < lowest) {
                lowest = found;
            }
...
let input = require('fs').readFileSync(__dirname + '/../__data__/day05.txt', 'utf-8');
let day05 = new Day05(input);
console.log("Day 05, Part 1", day05.process());