...
const benchmark = new Benchmarkify("Day 01 - Advent of Code 2023").printHeader();
...
let ap = new AsyncCalibrationParser(input);

benchmark.createSuite('Day 1', { time: 1000, description: 'Day 1' })
    .setup(() => {
        p = new CalibrationParser(input);
        rp = new ReplacementCalibrationParser(input);
        ap = new AsyncCalibrationParser(input);
    })
    .add("CalibrationParser", () => {
        p.getSumOfCalibrationValues();
    })
    .add("ReplacementCalibrationParser", () => {
        rp.getSumOfCalibrationValues();
    })
    .add("AsyncCalibrationParser", (done: () => void) => {
        ap.getSumOfCalibrationValues().then(() => { done(); });
    })
